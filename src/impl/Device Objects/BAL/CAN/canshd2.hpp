...
/// </example>
//*****************************************************************************
public ref class  CanCyclicTXMsg2 sealed
  : public ICanCyclicTXMsg2
{
  //--------------------------------------------------------------------
  // member variables
  //--------------------------------------------------------------------
  internal:
    CanScheduler2^      m_pCanShd; // pointer to the native scheduler object
    mgdCANCYCLICTXMSG2  m_CanMsg;  // cyclic CAN message structure
    ...
    //--------------------------------------------------------------------
    // ICanCyclicTXMsg2 implementation
    //--------------------------------------------------------------------
  public:
    //*****************************************************************************
    /// <summary>
    ...
    ///   the properties <c>ICanSocket2.CanClockFrequency</c> and 
    ///   <c>ICanSocket2.TimeStampCounterDivisor</c> in accordance with the 
    ///   following formula:
    ///   <code>
    ///     Resolution [s] = TimeStampCounterDivisor / CanClockFrequency 
    ///   </code>
    ...
    ///   properties <c>ICanSocket2.CanClockFrequency</c> and 
    ///   <c>ICanSocket2.DelayedTXTimerDivisor</c> in accordance with the following 
    ///   formula:
    ///   <code>
    ///     delay time [s] = (DelayedTXTimerDivisor / CanClockFrequency) * TimeStamp 
    ///   </code>
    ///   The maximum possible delay time is defined by the property 
    ///   <c>ICanSocket2.MaxDelayedTXTicks</c>.
    /// </summary>
    //*****************************************************************************
    ...
    virtual property Byte DataLength 
    {
      ...
      {
        pin_ptr<mgdCANMSGINFO> pMngtInfo = &m_CanMsg.uMsgInfo;
        return( can_dlc2len[((PCANMSGINFO)pMngtInfo)->Bits.dlc] );
      };
      ...
      {
        if (length <= CAN_ELEN_MAX)
        {
          pin_ptr<mgdCANMSGINFO> pMngtInfo = &m_CanMsg.uMsgInfo;
          ((PCANMSGINFO)pMngtInfo)->Bits.dlc = can_len2dlc[length];
        }
        else
        {
    ...
    virtual property Byte default[int] 
    {
      ...
      {
        if ((index >= 0) && (index < CAN_ELEN_MAX))
        {
          pin_ptr<mgdCANCYCLICTXMSG2> pMngtMsg = &m_CanMsg;
          return( ((PCANCYCLICTXMSG2)pMngtMsg)->abData[index] );
        }
        else
        {
      ...
      void set(Int32 index, Byte value) sealed
      {
        ...
        {
          pin_ptr<mgdCANCYCLICTXMSG2> pMngtMsg = &m_CanMsg;
          ((PCANCYCLICTXMSG2)pMngtMsg)->abData[index] = value;
        }
        else
        {
          throw gcnew ArgumentOutOfRangeException("index");
  ...
  public:
    ...
    virtual bool Equals(Object^ obj) override
    {
      ...
      CanCyclicTXMsg2^ p = (CanCyclicTXMsg2^)obj;
      size_t length = sizeof(mgdCANCYCLICTXMSG2);
      if (CanMsgFrameType::Data == p->FrameType)
      {
        length += p->DataLength;
      }

      mgdCANCYCLICTXMSG2 msg = p->m_CanMsg;
      pin_ptr<mgdCANCYCLICTXMSG2> pMsg1 =  &msg;
      pin_ptr<mgdCANCYCLICTXMSG2> pMsg2 =  &m_CanMsg;
  ...
  public:
    CanCyclicTXMsg2  ( CanScheduler2^ pSched );
};
...
/// </summary>
//*****************************************************************************
private ref class CanScheduler2 : public CanSocket2
                                , public ICanScheduler2
{
  //--------------------------------------------------------------------
  // member variables
  //--------------------------------------------------------------------
  private:
    ::ICanScheduler2*          m_pCanShd;  // native scheduler object
    array<CanCyclicTXMsg2^>^   m_aCtxMsg;  // array for cyclic TX messages
  ...
  //--------------------------------------------------------------------
  private:
    HRESULT InitNew           ( ::ICanScheduler2* pCanShd );
    void    Cleanup           ( void );
    void    ResetScheduler    ( void );

  internal:
    CanScheduler2    ( ::IBalObject* pBalObj
                     , Byte          portNumber 
                     , Byte          busTypeIndex);
   ~CanScheduler2    ( void );
  ...
  public:
    ...
    virtual ICanCyclicTXMsg2^ AddMessage( void );

  internal:
    void InternalAddMessage( CanCyclicTXMsg2^ cyclicTXMessage );
    void InternalRemMessage  ( CanCyclicTXMsg2^ cyclicTXMessage );
    void InternalStartMessage( CanCyclicTXMsg2^ cyclicTXMessage, UInt16 repeatCount );
    void InternalStopMessage ( CanCyclicTXMsg2^ cyclicTXMessage );
};
